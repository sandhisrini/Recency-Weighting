...
from score_functions import score_12, score_12_variant_a
...
parser.add_argument("--args", "-a", default=1.0, type=float)
parser.add_argument("--alpha", "-A", default=0.5, type=float, help="Weight for confidence term a in score_12")
parser.add_argument("--lmbda", "-L", default=1.0, type=float, help="Lambda for time decay in score_12")
...
args_ratio = parsed["args"]
alpha = parsed.get("alpha", 0.5)
lmbda = parsed.get("lmbda", 1.0)
...
dir_path = os.path.join(script_dir, "output", dataset) + "/"

# If no rules file specified, find the most recent .json file in the output directory
if not rules_file:
    json_files = [f for f in os.listdir(dir_path) if f.endswith('.json')]
    if not json_files:
        raise FileNotFoundError(f"No .json rules file found in {dir_path}. Please provide --rules argument.")
    rules_file = sorted(json_files)[-1]  # Use the most recent file
...
# Use Variant A: Recency-weighted scoring
score_func = score_12_variant_a
# It is possible to specify a list of list of arguments for tuning
# Default per paper: lmbda=1.0, a=0.5. The legacy --args flag set a; keep for backward compatibility only if explicitly changed.
if "args" in parsed and parsed["args"] != 1.0:
    alpha = args_ratio
args = [[lmbda, alpha]]
    ...
    for j in test_queries_idx:
        ...
        if test_query[1] in rules_dict:
            ...
            for rule in rules_dict[test_query[1]]:
                if rule["acyclic"] == False:
                    walk_edges = ra.match_body_relations(rule, edges, test_query[0])
                else:
                    walk_edges = ra.match_acyclic_bodies(rule, edges, test_query[0])
                if 0 not in [len(x) for x in walk_edges]:
                    ...
                    if not rule_walks.empty:
                        ...
                        for s in dicts_idx:
                            cands_dict[s] = {
                                x: sorted(cands_dict[s][x], reverse=True)       #candidates scores sorted
                                for x in cands_dict[s].keys()
                            }
                            cands_dict[s] = dict(
                                sorted(
                                    cands_dict[s].items(),
                                    key=lambda item: item[1],           #candidates sorted according to scores
                                    reverse=True,
                                )
                            )
