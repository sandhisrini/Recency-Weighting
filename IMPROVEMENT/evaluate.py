...
parser.add_argument("--original_only", action="store_true",
                    help="Evaluate only the original test queries (exclude inverse duplicates)")
parsed = vars(parser.parse_args())


def filter_candidates(test_query, candidates, all_facts):
    """
    ...
    # Time-aware filtering: remove all other true objects at the same timestamp
    # Must consider the entire KG at time t (train+valid+test), not just test split
    other_answers = all_facts[
        (all_facts[:, 0] == test_query[0])
        * (all_facts[:, 1] == test_query[1])
        * (all_facts[:, 2] != test_query[2])
        * (all_facts[:, 3] == test_query[3])
    ]
...
# If no candidates file specified, find the most recent _cands_ .json file in the output directory
if not candidates_file:
    json_files = [f for f in os.listdir(dir_path) if f.endswith('.json') and '_cands_' in f]
    if not json_files:
        raise FileNotFoundError(f"No candidates .json file found in {dir_path}. Please provide --candidates argument.")
    candidates_file = sorted(json_files)[-1]  # Use the most recent file
...
for i in range(num_samples):
    ...
    candidates = filter_candidates(test_query, candidates, data.all_idx)
